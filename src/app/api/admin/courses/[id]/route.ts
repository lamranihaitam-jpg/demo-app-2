...
export async function PATCH(req: Request, { params }: { params: { id: string } }){
  try{
    ...
    const prisma = getPrisma()
    // simple strategy: update basic fields and if sessions provided, delete existing sessions and recreate (keeps logic simple)
    await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      await tx.course.update({ where: { id }, data: { title, description, image } })
      if (Array.isArray(sessions)){
        // delete previous sessions + slots
        await tx.session.deleteMany({ where: { courseId: id } })
        // create new sessions
        for (const s of sessions){
          const created = await tx.session.create({ data: { courseId: id, startDate: new Date(s.startDate), endDate: s.endDate ? new Date(s.endDate) : undefined, location: s.location } })
          if (Array.isArray(s.slots) && s.slots.length>0){
            await tx.slot.createMany({ data: s.slots.map((sl:any)=>({ sessionId: created.id, time: sl.time, capacity: sl.capacity ?? 1 })) })
          }
        }
      }
    })

    const updated = await prisma.course.findUnique({ where: { id }, include: { sessions: { include: { slots: true } } } })
